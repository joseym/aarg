...
/// Ask whether a credential is a pay-as-you-go API key or a Claude plan
/// (OAuth) token. The `_` arm covers the API-key option and any unexpected
/// value, so there's no panicking catch-all.
fn prompt_auth_kind() -> Result<AuthKind, CliError> {
    const API: &str = "API key (pay-as-you-go)";
    const SUB: &str = "Claude subscription / Pro or Max (experimental)";
    match Select::new("Credential type:", vec![API, SUB]).prompt()? {
        SUB => Ok(AuthKind::Oauth),
        _ => Ok(AuthKind::ApiKey),
    }
}

/// Prompt for a masked secret of `kind`, store it under `label`, and make
/// that label active. The secret is read masked, never echoed, and goes
/// straight to the OS keychain — never into the config file. When stdin is
/// not a terminal (scripts, CI), inquire fails with a typed error rather
...
async fn add_key(
    ...
    println!("{noun} `{label}` stored in the OS keychain and set active.");
    Ok(())
}
